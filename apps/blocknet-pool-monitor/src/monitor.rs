...
use pool_runtime::service_state::{PersistedRuntimeSnapshot, LIVE_RUNTIME_SNAPSHOT_META_KEY};
...
const MONITOR_LISTEN_ADDR: &str = "127.0.0.1:24784";
const MONITOR_INTERVAL: Duration = Duration::from_secs(10);
const MONITOR_SPOOL_PATH: &str = "/var/lib/blocknet-pool/monitor-spool.jsonl";
...
const HEIGHT_DIVERGENCE_THRESHOLD: u64 = 2;
const SEED_STATUS_URLS: &[&str] = &[
    "http://bnt-0.blocknetcrypto.com:28081/status",
    "http://bnt-1.blocknetcrypto.com:28081/status",
    "http://bnt-2.blocknetcrypto.com:28081/status",
    "http://bnt-3.blocknetcrypto.com:28081/status",
    "http://bnt-4.blocknetcrypto.com:28081/status",
];
const POOL_STATUS_URL: &str = "https://bntpool.com/api/status";
const EXPLORER_STATUS_URL: &str = "https://explorer.blocknetcrypto.com/status.json";
...
struct MonitorSnapshot {
    ...
    oldest_pending_payout_age_seconds: Option<u64>,
    oldest_pending_send_age_seconds: Option<u64>,
    validation_candidate_queue_depth: Option<u64>,
    validation_regular_queue_depth: Option<u64>,
    reference_height_spread: Option<u64>,
    reference_height_min: Option<u64>,
    reference_height_max: Option<u64>,
    reference_heights: BTreeMap<String, u64>,
    reference_source_up: BTreeMap<String, bool>,
    api_performance: ApiPerformanceSnapshot,
    stratum_runtime_tasks: BTreeMap<String, TimedOperationSummary>,
    process_metrics: BTreeMap<String, ProcessMetrics>,
    open_public_incidents: u64,
    open_private_incidents: u64,
}
...
pub(crate) async fn run_monitor(config_path: &Path) -> Result<()> {
    load_dotenv(config_path);
    let cfg = Config::load(config_path)?;
    let cfg_for_store = cfg.clone();
    let store = tokio::task::spawn_blocking(move || {
        PoolStore::open(
            &cfg_for_store.runtime.database_url,
            cfg_for_store.runtime.database_pool_size,
        )
    })
    .await
    .context("join monitor store initialization task")??;
    let daemon_api = cfg.runtime.daemon_api.clone();
    let daemon_cookie_path = cfg.runtime.daemon_cookie_path.clone();
    let node =
    ...
    let monitor = Arc::new(MonitorRuntime {
        ...
        spool_path: PathBuf::from(MONITOR_SPOOL_PATH),
...
impl MonitorRuntime {
    fn listen_addr(&self) -> SocketAddr {
        MONITOR_LISTEN_ADDR
            .parse()
            .expect("monitor listen address constant should parse")
    }
    ...
    async fn monitor_loop(self: Arc<Self>) -> Result<()> {
        ...
        loop {
            ...
            if let Err(err) = self.persist_sample(&sample).await {
                ...
                self.update_metrics(&sample).await;
    ...
    async fn collect_sample(&self, sampled_at: SystemTime) -> MonitorSample {
        ...
        let validation_backlog = validation_backlog_state(
            ...
            validation_queue_capacity(&self.cfg),
        );
        ...
        let reference_height_divergence =
            reference_height_divergence_state(&reference_heights, HEIGHT_DIVERGENCE_THRESHOLD);
        let summary_state = summarize_state(SummaryStateInput {
            api_up: api.error.is_none(),
            stratum_up: stratum.error.is_none() && stratum_snapshot_fresh,
            db_up: db.error.is_none(),
            daemon_ready: daemon.value.as_ref().is_some_and(|status| !status.syncing),
            template_refresh_millis,
            daemon_slow_block_severity: daemon_slow_block.as_ref().map(|(severity, _)| *severity),
            pool_activity_loss_severity: pool_activity_lost.as_ref().map(|(severity, _)| *severity),
            validation_backlog_severity: validation_backlog.as_ref().map(|(severity, _)| *severity),
            payout_queue_severity: payout_queue_stalled.as_ref().map(|(severity, _)| *severity),
            share_ingest_severity: share_ingest_stalled.as_ref().map(|(severity, _)| *severity),
            reference_height_divergence_severity: reference_height_divergence
                .as_ref()
    ...
    async fn sync_incidents(&self, sample: &MonitorSample) -> Result<()> {
        ...
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_api_down",
                kind: "api_down",
                severity: "critical",
                visibility: "public",
                active: sample.api_probe.error.is_some(),
                threshold: Duration::from_secs(20),
                detail: sample.api_probe.error.clone(),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_database_down",
                kind: "pool_database_down",
                severity: "critical",
                visibility: "public",
                active: sample.db_probe.error.is_some(),
                threshold: Duration::from_secs(20),
                detail: sample.db_probe.error.clone(),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_stratum_down",
                kind: "stratum_down",
                severity: "critical",
                visibility: "public",
                active: sample.stratum_probe.error.is_some() || snapshot_stale,
                threshold: STRATUM_SNAPSHOT_STALE_AFTER,
                detail: sample
                    .stratum_probe
                    .error
                    .clone()
        ...
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_pool_activity_lost",
                kind: "pool_activity_lost",
                severity: pool_activity_lost
                    .as_ref()
        ...
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_daemon_down",
                kind: "daemon_down",
                severity: "critical",
                visibility: "public",
                active: sample.daemon_status.error.is_some(),
                threshold: Duration::from_secs(30),
                detail: sample.daemon_status.error.clone(),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_daemon_syncing",
                kind: "daemon_syncing",
                severity: "warn",
                visibility: "public",
                active: daemon_syncing,
                threshold: Duration::from_secs(0),
                detail: Some("daemon reported syncing".to_string()),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_daemon_slow_block_processing",
                kind: "daemon_slow_block_processing",
                severity: daemon_slow_block
                    .as_ref()
                    .map(|(severity, _)| *severity)
                    .unwrap_or("warn"),
                visibility: "public",
                active: daemon_slow_block.is_some(),
                threshold: Duration::from_secs(0),
                detail: daemon_slow_block.map(|(_, detail)| detail),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_template_stale",
                kind: "template_stale",
                severity: if template_refresh_millis
        ...
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_validation_backlog",
                kind: "validation_backlog",
                severity: validation_backlog
                    .as_ref()
                    .map(|(severity, _)| *severity)
                    .unwrap_or("warn"),
                visibility: "private",
                active: validation_backlog.is_some(),
                threshold: Duration::from_secs(60),
                detail: validation_backlog.map(|(_, detail)| detail),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_payout_queue_stalled",
                kind: "payout_queue_stalled",
                severity: payout_queue_stalled
                    .as_ref()
                    .map(|(severity, _)| *severity)
                    .unwrap_or("warn"),
                visibility: "private",
                active: payout_queue_stalled.is_some(),
                threshold: Duration::from_secs(0),
                detail: payout_queue_stalled.map(|(_, detail)| detail),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_wallet_unavailable",
                kind: "wallet_unavailable",
                severity: "warn",
                visibility: "private",
                active: sample.wallet_probe.error.is_some(),
                threshold: Duration::from_secs(10 * 60),
                detail: sample.wallet_probe.error.clone(),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "local_share_ingest_stalled",
                kind: "share_ingest_stalled",
                severity: share_ingest_stalled
                    .as_ref()
                    .map(|(severity, _)| *severity)
                    .unwrap_or("warn"),
                visibility: "private",
                active: share_ingest_stalled.is_some(),
                threshold: Duration::from_secs(0),
                detail: share_ingest_stalled.map(|(_, detail)| detail),
            },
            now,
            &mut actions,
        )?;
        self.sync_condition(
            ...
            IncidentCondition {
                dedupe_key: "external_chain_height_divergence",
                kind: "chain_height_divergence",
                severity: "critical",
                visibility: "public",
                active: reference_height_divergence.is_some(),
                threshold: Duration::from_secs(0),
                detail: reference_height_divergence
                    .as_ref()
    ...
    fn sync_condition(
        ...
        if condition.active {
            ...
            if now.duration_since(started_at).unwrap_or_default() >= condition.threshold {
                actions.push(IncidentStoreAction::Upsert(MonitorIncidentUpsert {
                    dedupe_key: condition.dedupe_key.to_string(),
                    kind: condition.kind.to_string(),
                    severity: condition.severity.to_string(),
                    visibility: condition.visibility.to_string(),
                    source: LOCAL_MONITOR_SOURCE.to_string(),
                    summary: condition
                        .detail
                        .clone()
                        .unwrap_or_else(|| condition.kind.replace('_', " ")),
        ...
        let dedupe_key_owned = condition.dedupe_key.to_string();
            ...
            if now.duration_since(started_at).unwrap_or_default() >= condition.threshold {
                actions.push(IncidentStoreAction::Upsert(MonitorIncidentUpsert {
                    ...
                    summary: condition
                        .detail
                        .clone()
                        .unwrap_or_else(|| format!("{} recovered", condition.kind)),
                    detail: condition.detail,
                    started_at,
                    updated_at: now,
                }));
    ...
    async fn probe_stratum(&self) -> ProbeOutcome<()> {
        let target = format!(
            "{}:{}",
            self.cfg.runtime.stratum_host, self.cfg.runtime.stratum_port
        );
    ...
    async fn probe_reference_heights(&self) -> Vec<ReferenceHeightSample> {
        ...
        {
            let client = self.external_client.clone();
            let url = POOL_STATUS_URL.to_string();
            tasks.push(tokio::task::spawn_blocking(move || {
        ...
        {
            let client = self.external_client.clone();
            let url = EXPLORER_STATUS_URL.to_string();
            tasks.push(tokio::task::spawn_blocking(move || {
    ...
    async fn notify_chain_divergence(&self, transition: ChainDivergenceTransition, detail: String) {
        ...
        let client = self.external_client.clone();
        let threshold = HEIGHT_DIVERGENCE_THRESHOLD;
    ...
    fn local_api_url(&self, path: &str) -> String {
        format!("http://{}:{}{path}", self.cfg.api_host, self.cfg.api_port)
    }
}
...
fn summarize_state(input: SummaryStateInput) -> &'static str {
    ...
    if input
        .template_refresh_millis
        .is_some_and(|lag| lag >= TEMPLATE_REFRESH_WARN_AFTER.as_millis() as u64)
        || input.daemon_slow_block_severity.is_some()
        || input.pool_activity_loss_severity.is_some()
        || input.validation_backlog_severity.is_some()
        || input.payout_queue_severity.is_some()
...
fn validation_backlog_state(
    ...
    let audit = snapshot.validation.audit_queue_depth as u64;
    let cap = max_queue.max(1);
    let max_depth = candidate.max(regular).max(audit);
    let combined = candidate.saturating_add(regular).saturating_add(audit);
    ...
    Some((
        severity,
        format!(
            "validation backlog candidate={} regular={} audit={} cap={}",
            candidate, regular, audit, cap
        ),
    ))
}
...
mod tests {
    ...
    fn healthy_summary_input() -> SummaryStateInput {
        SummaryStateInput {
            api_up: true,
            stratum_up: true,
            db_up: true,
            daemon_ready: true,
            template_refresh_millis: None,
            daemon_slow_block_severity: None,
            pool_activity_loss_severity: None,
            validation_backlog_severity: None,
            payout_queue_severity: None,
            share_ingest_severity: None,
    ...
    fn payout_queue_stall_degrades_summary_state() {
        let state = summarize_state(SummaryStateInput {
            payout_queue_severity: Some("critical"),
            ..healthy_summary_input()
        });
    ...
    fn pool_activity_loss_marks_summary_degraded() {
        let state = summarize_state(SummaryStateInput {
            pool_activity_loss_severity: Some("critical"),
            ..healthy_summary_input()
        });
        assert_eq!(state, "degraded");
    }
