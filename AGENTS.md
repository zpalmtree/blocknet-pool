...
- The frontend bundle is built locally into `frontend/dist/` and then embedded into the API binary during the local release build, so frontend-only changes still require an API rebuild/restart.
