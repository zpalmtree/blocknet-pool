...
use pool_common::pow::difficulty_to_target;
...
pub(crate) enum AssignmentRangeMode {
    Fresh,
    PreserveCurrent,
}
...
impl JobManager {
    ...
    pub(crate) fn build_miner_job_with_range_mode(
        &self,
        conn_id: &str,
        share_difficulty: u64,
        assigned_miner: &str,
    ...
    fn recover_wallet_loaded_and_unlocked(&self, password: &str) -> bool {
        ...
        if let Err(unlock_err) = self.node.wallet_unlock(password) {
            if is_http_status(&unlock_err, 503)
                ...
                && self.node.wallet_unlock(password).is_ok()
            {
                return true;
            }
            tracing::warn!(
...
fn same_template_identity(current: &Job, parsed: &Job) -> bool {
    current.height == parsed.height
        ...
        && match (current.prev_hash, parsed.prev_hash) {
            (Some(a), Some(b)) => a == b,
            _ => true,
        }
}
...
fn hex_decode(input: &str) -> anyhow::Result<Vec<u8>> {
    hex::decode(input.trim()).map_err(|err| match err {
        hex::FromHexError::OddLength => anyhow::anyhow!("hex length must be even"),
        _ => anyhow::anyhow!("invalid hex"),
    })
}
...
mod tests {
    ...
    fn test_job(id: &str, height: u64) -> Job {
        Job {
            id: id.to_string(),
            height,
    ...
    fn miner_job_carries_targets_and_nonce_window() {
...

        {
            ...
            state.current = Some(test_job("job1", 1));
        }
    ...
    fn prune_expired_assignments_keeps_recently_stale_previous_template_assignments() {
        ...
        state
            .jobs
            .insert("job-old".to_string(), test_job("job-old", 10));
        state.job_meta.insert(
            "job-old".to_string(),
            JobTemplateMeta {
    ...
    fn resolve_submit_job_survives_prune_with_recent_stale_template_grace() {
        ...
        let old_job = test_job("job-old", 10);
        let current_job = test_job("job-current", 11);
    ...
    fn resolve_submit_job_reports_expired_assignment_reason() {
        ...
        let old_template = test_job("job-old", 1);
        let current_template = test_job("job-current", 2);
    ...
    fn resolve_submit_job_allows_recent_previous_template_within_grace() {
        ...
        let old_job = test_job("job-old", 10);
        let new_job = test_job("job-new", 11);
    ...
    fn template_identity_ignores_template_churn_fields() {
        ...
        let mut missing_prev_hash = base.clone();
        missing_prev_hash.prev_hash = None;
        assert!(same_template_identity(&base, &missing_prev_hash));

        let mut different_prev_hash = base.clone();
        different_prev_hash.prev_hash = Some([0xBB; 32]);
