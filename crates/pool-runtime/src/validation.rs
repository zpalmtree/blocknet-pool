...
const AUDIT_MAX_ADDRESSES_PER_TICK: usize = 4;
const AUDIT_MAX_SHARES_PER_ADDRESS: usize = 8;
...
const FORCED_VALIDATION_QUARANTINE_THRESHOLD: f64 = 0.20;
const PROVISIONAL_BACKLOG_VERIFIED_MULTIPLIER: f64 = 8.0;
const CANDIDATE_VALIDATION_WORKERS: usize = 1;
...
pub(crate) enum ValidationFollowupAction {
    #[default]
    None,
    Quarantine,
}
...
#[derive(Debug, Clone)]
pub(crate) struct PersistedValidationProvisional {
    pub address: String,
...
#[derive(Debug, Clone)]
pub(crate) struct PersistedValidationAcceptedShare {
    pub address: String,
    pub share_id: i64,
    pub created_at: SystemTime,
    pub difficulty: u64,
...
#[derive(Debug, Clone)]
pub(crate) struct ValidationClearEvent {
    pub id: i64,
    pub address: String,
...
pub(crate) struct LoadedValidationState {
...
pub(crate) trait ValidationStateStore: Send + Sync + 'static {
...
#[serde(rename_all = "snake_case")]
pub(crate) enum ValidationAuditSource {
    Sampled,
    ForcedReview,
    PayoutCoverage,
    ProvisionalBacklog,
...
#[derive(Debug, Clone, Copy)]
pub(crate) struct RegularSubmitPlan {
...
pub(crate) struct ValidationEngine {
...
impl ValidationEngine {
    #[cfg(test)]
    pub(crate) fn new(config: Config, hasher: Arc<dyn PowHasher>) -> Self {
    ...
    pub(crate) fn new_with_state_store(
        ...
        tracing::info!(
            candidate_validation_workers = candidate_workers,
            regular_validation_workers = regular_workers,
    ...
    pub(crate) fn snapshot(&self) -> ValidationSnapshot {
        self.inner.snapshot()
    }

    pub(crate) fn schedule_forced_review_after(&self, address: &str, start_at: SystemTime) {
...
impl ValidationInner {
    ...
    fn plan_regular_submit(&self, address: &str) -> RegularSubmitPlan {
            ...
            && should_force_for_payout_coverage(&effective, &self.config, now)
        {
            ...
            let next_share_idx = effective.total_shares.saturating_add(1);
            if (self.config.warmup_shares > 0 && next_share_idx <= self.config.warmup_shares as u64)
                || (self.config.min_sample_every > 0
                    && next_share_idx.is_multiple_of(self.config.min_sample_every as u64))
            {
    ...
    fn finalize_result(
        ...
        let followup_action = self.update_address_state(AddressStateUpdate {
            address: &task.address,
            share_id: None,
            created_at: SystemTime::now(),
            difficulty: task.difficulty,
            accepted: computed.accepted,
            sampled: computed.verified,
            invalid_sample,
            provisional_accepted,
            overload_mode: computed.overload_mode,
        });
    ...
    fn update_address_state(&self, update: AddressStateUpdate<'_>) -> ValidationFollowupAction {
        let now = SystemTime::now();
        let mut state = self.state.lock();
        let st = get_or_insert_state(&mut state, update.address);
        let forced_review_duration = self.config.forced_verify_duration();
        ...
        if update.accepted {
            st.accepted_window.push_back(AcceptedWindowShare {
                share_id: update.share_id.unwrap_or_default(),
                created_at: update.created_at,
                difficulty: update.difficulty.max(1),
                verified: update.sampled,
            });
        }
        if update.provisional_accepted {
        ...
        let followup = self.evaluate_post_sample_state(
            st,
            now,
            update.overload_mode,
            update.sampled,
            update.invalid_sample,
        );

        let persisted = persisted_validation_state(update.address, st);
        let provisional_to_persist =
            update
                .provisional_accepted
                .then_some(TrackedProvisionalShare {
                    share_id: update.share_id,
                    created_at: update.created_at,
                });
    ...
    fn evaluate_post_sample_state(
        ...
        let min_invalids = INVALID_SAMPLE_COUNT_THRESHOLD;
        let forced_clear_threshold = self.config.invalid_sample_threshold;
        let forced_quarantine_threshold = FORCED_VALIDATION_QUARANTINE_THRESHOLD;
    ...
    fn process_audit_task(&self, queued: QueuedAuditTask, started_at: Instant) {
        let task = ValidationTask {
            ...
            share_target: pool_common::pow::difficulty_to_target(queued.share.difficulty.max(1)),
    ...
    fn schedule_background_audits(&self, now: SystemTime) {
        ...
        let provisional_cutoff = now
            ...
            .unwrap_or(now);
        let max_addresses = AUDIT_MAX_ADDRESSES_PER_TICK;
        let per_address_limit = AUDIT_MAX_SHARES_PER_ADDRESS;
    ...
    fn effective_sample_rate(&self, overload_mode: OverloadMode) -> f64 {
        ...
        match overload_mode {
            ...
            OverloadMode::Shed => base.min(OVERLOAD_SAMPLE_RATE_FLOOR),
            ...
            OverloadMode::Shed | OverloadMode::Emergency => {
                if below_clear {
                    if let Some(started_at) = state.below_clear_started_at {
                        if now.saturating_duration_since(started_at) >= OVERLOAD_CLEAR_HOLD {
    ...
    fn record_hot_accept(
        ...
        let _ = self.update_address_state(AddressStateUpdate {
            address,
...
fn payout_coverage_target_ratio(config: &Config) -> Option<f64> {
    if config.payout_provisional_cap_multiplier > 0.0 {
...
fn payout_coverage_window_start(config: &Config, now: SystemTime) -> Option<SystemTime> {
    let duration = config.pplns_window_duration();
...
fn should_force_for_provisional_backlog(
    ...
    stats.recent_provisional_difficulty as f64
        > (stats.recent_verified_difficulty as f64 * PROVISIONAL_BACKLOG_VERIFIED_MULTIPLIER)
            + f64::EPSILON
}
...
fn load_initial_validation_state(
    ...
    for provisional in loaded.provisionals {
        let entry = state.entry(provisional.address.clone()).or_default();
    ...
    for accepted in loaded.accepted_window {
        let entry = state.entry(accepted.address.clone()).or_default();
        entry.accepted_window.push_back(AcceptedWindowShare {
...
mod tests {
    ...
    use pool_common::db::{DbBlock, ShareReplayData};
    use pool_common::pow::{difficulty_to_target, DeterministicTestHasher};
    ...
    fn invalid_sample_escalates_force_verify() {
        ...
        for _ in 0..INVALID_SAMPLE_COUNT_THRESHOLD {
    ...
    fn invalid_sample_requires_count_threshold_before_force_verify() {
        ...
        for nonce in 11..(11 + INVALID_SAMPLE_COUNT_THRESHOLD - 1) {
        ...
        assert!(
            ...
            "invalid samples below the count threshold should not force verified-only mode"
        );

        let mut third_bad = matching_task(16);
        ...
        let after_threshold = engine.process_inline(matching_task(17));
        assert!(
    ...
    fn forced_review_only_quarantines_after_review_window_expires() {
        ...
        for _ in 0..INVALID_SAMPLE_COUNT_THRESHOLD {
            let first_result = engine.process_inline(first_bad.clone());
...

        {
            ...
            entry.forced_sampled_shares = INVALID_SAMPLE_COUNT_THRESHOLD;
            entry.forced_invalid_samples = INVALID_SAMPLE_COUNT_THRESHOLD;
    ...
    fn provisional_backlog_budget_forces_full_verify() {
        ...
        let mut task1 = matching_task(1);
        task1.difficulty = 5;
        let r1 = engine.process_inline(task1);
        assert!(r1.accepted);
        assert!(!r1.verified);

        let mut task2 = matching_task(2);
        task2.difficulty = 5;
        let r2 = engine.process_inline(task2);
        assert!(r2.accepted);
        assert!(!r2.verified);
    ...
    fn provisional_backlog_hold_clears_once_backlog_drains() {
        ...
        let mut task1 = matching_task(1);
        task1.difficulty = 5;
        let first = engine.process_inline(task1);
        let mut task2 = matching_task(2);
        task2.difficulty = 5;
        let second = engine.process_inline(task2);
    ...
    fn overload_mode_can_downgrade_from_emergency_to_shed() {
        let engine = ValidationEngine::new(test_cfg(), Arc::new(DeterministicTestHasher));
        let submit_queue = Arc::new(QueueTracker::new(32));
        engine.attach_submit_regular_queue(Arc::clone(&submit_queue));
