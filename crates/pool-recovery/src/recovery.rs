...
use crate::config::{Config, RecoveryConfig, RecoveryDaemonInstanceConfig};
use pool_runtime::node::{NodeStatus, WalletAddressResponse, WalletBalance};
...
impl RecoveryAgent {
    pub async fn new(cfg: Config) -> Result<Arc<Self>> {
        ...
        for operation in persisted.operations.iter_mut() {
            if operation.state == RecoveryOperationState::Running {
        ...
        Fut: Future<Output = Result<String>> + Send + 'static,
    {
        ...
        if state
            .operations
            .iter()
            .any(|op| op.state == RecoveryOperationState::Running)
        {
    ...
    async fn build_status_snapshot(&self) -> Result<RecoveryStatus> {
        let proxy_target = self.cfg.recovery.detect_proxy_target();
        let active_cookie_target = self.cfg.recovery.detect_active_cookie_target();
        let active_instance = RecoveryConfig::effective_active_instance_from_targets(
            proxy_target,
